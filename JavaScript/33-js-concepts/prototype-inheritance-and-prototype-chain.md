...
아래 코드는 `new` 연산자와 생성자 함수를 사용해 객체를 생성하는 예제이다.
...
`new` 연산자와 생성자 함수를 사용하면 다수의 유사한 객체를 쉽게 생성할 수 있지만, 생성자 함수를 사용해 생성된 모든 객체는 고유한 프로퍼티와 메소드의 복사본을 갖게 된다. 

프로토타입을 사용하면 이러한 메모리 낭비를 막을 수 있다.
...
자바스크립트에서 객체들은 `[[Prototype]]` 이라는 숨겨진 프로퍼티를 갖는다. 이 프로퍼티는 `null `값을 가지거나 다른 객체를 참조하는데, `[[Prototype]]` 프로퍼티에 의해 참조 대상이 되는 객체를 프로토타입(prototype)이라고 한다.
...
`__proto__` 대신  `Object.getPrototypeOf`나  `Object.setPrototypeOf`을 사용해 프로토타입을 get하거나 set할 수 있다.
...
`__proto__`의 값은 `null` 또는 다른 객체만 사용될 수 있으며 다른 타입들은 무시된다. 또한 객체는 두개 이상의 객체를 상속받지 못하고 오직 하나의 `[[Prototype]]`만 가질 수 있다.
...
객체에서 호출된 메소드인지 혹은 프로토타입에서 호출된 메소드인지 관계없이 호출된 메소드의 `this`의 값은  `.`(dot) 왼쪽에 있는 객체이다.

많은 메소드를 가지고 있는 큰 객체를 만들고 여러 객체가 이 큰 객체를 상속받게 하는 경우, 상속받은 메소드를 사용하게 되더라도 객체는 프로토타입 객체가 아닌 자신의 객체를 변경한다.
...
alert(rabbit.isSleeping); // true, this = rabbit
/*
rabbit: 
...
상속받은 메소드의 `this`는 `animal`이 아닌 호출될때 `.` 왼쪽에 있는 객체가 된다. 따라서 `this`에 데이터를 쓰면 해당 객체(`rabbit`)에 저장된다.

결과적으로, 메소드는 공유되지만 객체의 상태는 공유되지 않는다.
...
let rabbit = new Rabbit(); // {constructor: Rabbit}부터 상속된다.
